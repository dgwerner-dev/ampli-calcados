          ...
          <div class="flex items-center justify-between">
            <div class="flex items-center space-x-3">
              ...
              <div>
                ...
                <p class="text-coral-100 text-sm">Defina novos preços para todos os produtos de uma categoria</p>
              </div>
            </div>
            <button @click="closeModal" class="text-white/80 hover:text-white transition-colors p-2 hover:bg-white/10 rounded-full">
        ...
        <div class="bg-gray-50 px-6 py-8">
          ...
          <form @submit.prevent="handleSubmit" class="space-y-6">
            ...
            <div>
              ...
              <div class="space-y-3">
                <label class="flex items-center">
                  <input v-model="form.updateType" value="percentage" type="radio" name="updateType" class="h-4 w-4 text-coral-soft focus:ring-coral-soft border-gray-300" />
                  <span class="ml-2 text-sm text-gray-700">Percentual (%)</span>
                </label>
                <label class="flex items-center">
                  <input v-model="form.updateType" value="fixed" type="radio" name="updateType" class="h-4 w-4 text-coral-soft focus:ring-coral-soft border-gray-300" />
                  <span class="ml-2 text-sm text-gray-700">Valor Fixo (R$)</span>
                </label>
                <label class="flex items-center">
                  <input v-model="form.updateType" value="replace" type="radio" name="updateType" class="h-4 w-4 text-coral-soft focus:ring-coral-soft border-gray-300" />
            ...
            <div>
              ...
              <div class="relative">
                <span v-if="form.updateType !== 'percentage'" class="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-500">R$</span>
                <input
                  id="update-value"
                  v-model="form.value"
                  type="number"
                ...
                />
                <span v-if="form.updateType === 'percentage'" class="absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-500">%</span>
              </div>
              <p class="mt-2 text-xs text-gray-500">{{ getValueDescription() }}</p>
            </div>

            <!-- Preview -->
            <div v-if="previewProducts.length > 0" class="bg-white rounded-lg border border-gray-200 p-4">
              <h4 class="text-sm font-medium text-gray-900 mb-3">Preview dos Produtos Afetados</h4>
              <div class="space-y-2 max-h-40 overflow-y-auto">
                <div v-for="product in previewProducts" :key="product.id" class="flex items-center justify-between text-sm">
                  <span class="text-gray-700">{{ product.name }}</span>
                  <div class="flex items-center space-x-2">
                    <span class="text-gray-500 line-through">R$ {{ formatPrice(product.price) }}</span>
                    <span class="text-coral-soft font-medium">→ R$ {{ formatPrice(calculateNewPrice(product.price)) }}</span>
                  </div>
                </div>
              </div>
            ...
            <div class="flex items-center justify-end space-x-4 pt-6 border-t border-gray-200">
              <button type="button" @click="closeModal" class="px-6 py-3 border border-gray-300 text-gray-700 font-medium rounded-lg hover:bg-gray-50 transition-all duration-200">
                Cancelar
              </button>
              <button
                type="submit"
