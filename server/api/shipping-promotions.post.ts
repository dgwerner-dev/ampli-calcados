...
export default defineEventHandler(async event => {
  try {
    ...
    // Criar promoção usando Prisma
    const promotion = await prisma.shippingPromotion.create({
      data: {
        name,
        description,
    ...
    return {
      success: true,
      promotion,
      message: 'Promoção criada com sucesso! As informações de frete grátis foram atualizadas.',
    };
  } catch (error: any) {
